...
module.exports = function(redis) {
	...
	function QuerySwarm(id, query, worker, options) {
		...
		self.options = {
			...
			gracefulShutdown: typeof options.gracefulShutdown === 'boolean' ? options.gracefulShutdown : true,
		...
		if (self.options.gracefulShutdown) {
			process.on('SIGTERM', self.gracefulShutdown.bind(self));
		}
	...
	QuerySwarm.prototype.gracefulShutdown = function() {
		// allow workers to finish what they're doing
		QuerySwarm.prototype.stop(function(err) {
			if (err) return process.exit(1);
			// TODO replace with getProcessingLength once feature/status-api is merged
			return redis.llen(this.id + ':processing', function(err, len) {
				// confirm processing queue is empty and exit
				process.exit(len !== 0);
			});
		});
	};
