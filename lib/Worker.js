...
module.exports = function(redis) {
	...
	Worker.prototype.next = function(method, delay){
		...
		if (delay) {
			assert.equal(self.immediate, null);
			assert.equal(self.timeout, null);
			assert.equal(self._next.method, null);
			assert.equal(self._next.at, null);
			self._next = {
				method: method,
				at: delay ? Date.now() + delay : delay,
			};
		}
	...
	Worker.prototype.getNext = function(){
		return this.next;
	}; 
